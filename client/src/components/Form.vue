...
export default {
      ...
      if (this.formValid()) {
        try {
          ...
          this.openNotification(
            ...
            "Data, která jste zadali do formuláře, byla úspěšně uložena."
          );
        } catch (err) {
          console.log(err);
          this.openNotification(
            ...
            "top-right",
            "danger",
            "Data se nepodařilo uložit.",
            "Data, která jste zadali do formuláře, nebyla úspěšně uložena."
          );
        }
      } else {
        this.openNotification(
          ...
          "danger",
          "Chyba dat",
          "Ve formulářových datech je chyba, proto je nelze uložit."
        );
      }
    },
  ...
  watch: {
    config: {
      deep: true,
      handler: function() {
        this.changeMade = !_.isEqual(
          this.$store.state.editedConfig,
    ...
    changeMade(newValue) {
      if (newValue == true) {
        this.notification = this.openNotification(
          "none",
          "bottom-center",
          "dark",
          "Neuložené změny",
          "Provedli jste změny, které nebyly uloženy! Pro uložení stiskněte tlačítko pod formulářem."
        );
      } else {
